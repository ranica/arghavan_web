...
const mutations = {
    ...
    updateRecord: (state, payload) => {
        ...
        if (null != currentRecord){
            ...
            currentRecord.beginDate     = record.beginDate;
...
const actions = {
    ...
    loadRecords(context, page) {
        return new Promise((resolve, reject) => {
            axios.get('/contractors?page=' + page)
                .then(res => {
    ...
    saveRecord: (context, record) => {
        return new Promise((resolve, reject) => {
            // New record
            if (0 == record.id) {
                axios.post('/contractors', record)
                    .then(res => {
                        let status = (0 == res.data.status);
                        let newRecord = res.data.contractor;
            ...
            // Update Record
                ...
                axios.put('/contractors/' + record.id, record)
                    .then(res => {
                        let status = (0 == res.data.status);
                        let updatedRecord = res.data.CarColorcontractor;
