...
const actions = {
    ...
    loadRecords(context, page) {
        return new Promise((resolve, reject) => {
            axios.get('/permissions?page=' + page)
                .then(res => {
    ...
    saveRecord: (context, record) => {
        return new Promise((resolve, reject) => {
            // New record
            if (0 == record.id) {
                axios.post('/permissions', record)
                    .then(res => {
                        let status = (0 == res.data.status);
                        let newRecord = res.data.permission;
            ...
            // Update Record
                ...
                axios.put('/permissions/' + record.id, record)
                    .then(res => {
                        let status = (0 == res.data.status);
                        let updatedRecord = res.data.permission;
