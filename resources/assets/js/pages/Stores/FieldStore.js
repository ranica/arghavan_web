...
const mutations =
{
	...
	updateRecord: (state, payload) =>
	{
		...
		state._data.data[index].university = record.university;
	},
...
const actions =
{
	...
	saveRecord: (context, record) =>
	{
		return new Promise((resolve, reject) =>
		{
			...
			if (0 == record.id)
			{
				axios.post('/fields', record)
					.then(res =>
					{
						let status   = (0 == res.data.status);
						let newRecord = res.data.field;
			...
			{
				axios.put('/fields/' + record.id, record)
					.then(res =>
					{
						let status       = (0 == res.data.status);
