...
const actions = {
    ...
    saveRecord: (context, record) => {
        return new Promise((resolve, reject) => {
            // New record
            if (0 == record.id) {
                axios.post('/kintypes', record)
                    .then(res => {
                        let status = (0 == res.data.status);
                        let newRecord = res.data.kintype;
            ...
            // Update Record
                ...
                axios.put('/kintypes/' + record.id, record)
                    .then(res => {
                        let status = (0 == res.data.status);
                        let updatedRecord = res.data.kintype;
