...
const actions = {
	...
	loadRecords(context, page) {
		return new Promise((resolve, reject) => {
			axios.get('/gatepasses?page=' + page)
				.then(res => {
	...
	saveRecord: (context, record) => {
		return new Promise((resolve, reject) =>	{
			// New record
			if (0 == record.id)	{
				axios.post('/gatepasses', record)
					.then(res => {
						let status      = (0 == res.data.status);
						let newRecord = res.data.gatepass;
			...
			// Update Record
				...
				axios.put('/gatepasses/' + record.id, record)
					.then(res => {
						let status          = (0 == res.data.status);
						let updatedRecord = res.data.gatepass;
