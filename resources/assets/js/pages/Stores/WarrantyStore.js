...
const actions = {
	...
	loadRecords(context, page) {
		return new Promise((resolve, reject) => {
			axios.get('/warranties?page=' + page)
				.then(res => {
	...
	saveRecord: (context, record) =>
	{
		return new Promise((resolve, reject) => {
			// New record
			if (0 == record.id) {
				axios.post('/warranties', record)
					.then(res => {
						let status      = (0 == res.data.status);
						let newRecord 	= res.data.warranty;
			...
			// Update Record
				...
				axios.put('/warranties/' + record.id, record)
					.then(res => {
						let status          = (0 == res.data.status);
						let updatedRecord 	= res.data.warranty;
