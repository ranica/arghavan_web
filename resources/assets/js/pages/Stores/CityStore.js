...
const mutations = {
	...
	updateRecord: (state, payload) =>
	{
        ...
        if (null != currentRecord){
            ...
            currentRecord.province     = record.province;
        }
	},
...
const actions = {
	...
	saveRecord: (context, record) => {
		return new Promise((resolve, reject) =>	{
			// New record
			if (0 == record.id) {
				axios.post('/cities', record)
					.then(res => {
						let status   = (0 == res.data.status);
						let newRecord = res.data.city;
			...
			// Update record
				...
				axios.put('/cities/' + record.id, record)
					.then(res => {
						let status       = (0 == res.data.status);
