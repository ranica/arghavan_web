...
const mutations = {
	...
	updateRecord: (state, payload) => {
        ...
        if (null != currentRecord) {
			...
			currentRecord.gatedevices = record.gatedevices;
        }
	},
...
const actions = {
	...
	loadRecords(context, page) {
		return new Promise((resolve, reject) => {
			axios.get('/gategroups?page=' + page)
				.then(res => {
	...
	saveRecord: (context, record) => {
		return new Promise((resolve, reject) =>	{
			// New record
			if (0 == record.id)	{
				axios.post('/gategroups', record)
					.then(res => {
						let status   = (0 == res.data.status);
						let newRecord = res.data.gategroup;
			...
			// Update record
				...
				axios.put('/gategroups/' + record.id, record)
					.then(res => {
						let status       = (0 == res.data.status);
						let updatedRecord = res.data.gategroup;
	...
	saveGatedeviceRecord: (context, record) => {
		return new Promise((resolve, reject) =>	{
			let url = '/gategroups/' + record.gategroup_id + '/setGatedevice';
			let data = { gatedevices: record.gatedevices };

			axios.put(url, data)
				.then(res => {
					let status = (0 == res.data.status);
					let record = res.data.gategroup;
