...
const actions =
{
	...
	saveRecord: (context, record) =>
	{
		return new Promise((resolve, reject) =>
		{
			...
			if (0 == record.id)
			{
				axios.post('/resources', record)
					.then(res =>
					{
						let status   = (0 == res.data.status);
						let newRecord = res.data.resource;
			...
			{
				axios.put('/resources/' + record.id, record)
					.then(res =>
					{
						let status       = (0 == res.data.status);
						let updatedRecord = res.data.resource;
