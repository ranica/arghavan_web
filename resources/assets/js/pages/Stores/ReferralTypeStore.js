...
const actions = {
    ...
    loadRecords(context, page) {
        return new Promise((resolve, reject) => {
            axios.get('/referralTypes?page=' + page)
                .then(res => {
    ...
    saveRecord: (context, record) =>
    {
        return new Promise((resolve, reject) => {
            // New record
            if (0 == record.id) {
                axios.post('/referralTypes', record)
                    .then(res => {
                        let status      = (0 == res.data.status);
                        let newRecord   = res.data.referralType;
            ...
            // Update Record
                ...
                axios.put('/referralTypes/' + record.id, record)
                    .then(res => {
                        let status          = (0 == res.data.status);
                        let updatedRecord   = res.data.referralType;
