...
const getters = {
     ...
     */
    allRecords: state => state._all_contact_type,
};
...
const mutations = {
    ...
    /**
     * set all Contact Type
     */
    setAllContactTypes: (state, data) => state._all_contact_type = data,
    /**
     * Insert a Record
     */
...
const actions = {
    ...
    loadAllRecords(context, data) {
        return new Promise((resolve, reject) => {
            ...
            axios.get(url)
                .then(res => {
                    context.commit('setAllContactTypes', res.data);
    ...
    updateRecords(context, data) {
        return new Promise((resolve, reject) => {
            context.dispatch('updateModule/updateRecords', data)
                .then(res => {
                    let updatedRecord = res.data.contact_type;
    ...
    createRecords(context, data) {
        return new Promise((resolve, reject) => {
            context.dispatch('createModule/createRecords', data)
                .then(res => {
                    if (res.data.status == 0) {
                        context.commit('createRecord', res.data.contact_type);
                    }
