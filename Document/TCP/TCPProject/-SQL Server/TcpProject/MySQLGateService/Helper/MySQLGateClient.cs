...
namespace MySQLGateService.Helper
{
    public class MySQLGateClient
    {
        ...
        #region Methods
        public MySQLGateClient(string host, int port, int bufferSize)
        {
            prepare();
        ...
        private int ValidateData(int typeDirect, string ip, string commandGate)
        {
            ...
            switch (resultValidate)
            {
                case 0:
                    {
                        if (typeDirect == (int)Common.Enum.EDirection.input)
                            write(Common.Network.Command.SendReceiveData.sendDataInput);
                        else if (typeDirect == (int)Common.Enum.EDirection.output)
                            write(Common.Network.Command.SendReceiveData.sendDataOutput);
                ...
                case 11:
                    {
                        if (typeDirect == (int)Common.Enum.EDirection.input)
                            write(Common.Network.Command.SendReceiveData.sendDataDonotInput);
                        else if (typeDirect == (int)Common.Enum.EDirection.output)
                            write(Common.Network.Command.SendReceiveData.sendDataDonotOutput);
