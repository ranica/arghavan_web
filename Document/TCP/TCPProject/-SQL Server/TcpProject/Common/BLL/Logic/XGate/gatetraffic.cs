...
namespace Common.BLL.Logic.XGate
{
	...
	{
        #region Constants
        private const string C_spRegisterTarffic = "spRegisterTarffic";
        ...
        /// <returns></returns>
        public CommandResult RegisterTraffic(DataRow baseRecord, DataRow dataRecord, Dictionary<string, object> criterial, int message_id)
        {
            ...
            result = BaseDAL.DBaseHelper.executeCommand(BaseDAL.Base.EnumExecuteType.procedureNonQuery,
                connection, C_spRegisterTarffic, true,

                new KeyValuePair("@user_id", dataRecord["user_id"]),
                ...
                new KeyValuePair("@gatedirect_id", (int)criterial["direct"]),
                new KeyValuePair("@gatemessage_id", message_id),
                new KeyValuePair("@gateoperator_id", (int)criterial["srerviceId"]),
