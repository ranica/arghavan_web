...
{
	[Serializable]
	public class zone : BaseBLL.Entity.BaseByViewId
	{
		
	// Genereted Property of gatedevice
	//
	#region Relation - gatedevice (Has-Many relation)
		private System.Data.DataTable _get_gatedevice_zone_id;
		public System.Data.DataTable getgatedevice_zone_id
		{
			get
			{
				if ((_get_gatedevice_zone_id == null) && (AutoLoadForeignKeys))
					loadgatedevice_zone_id ();

				return _get_gatedevice_zone_id;
			}
			set
			{
				_get_gatedevice_zone_id	= value;
			}
		}

		public void loadgatedevice_zone_id (int pageIndex = -1, int pageSize = 100)
		{
			CommandResult	opResult;

			BLL.Logic.XGate.gatedevice	logic	= new BLL.Logic.XGate.gatedevice ("XGate");
			if (pageIndex == -1)
				opResult	= logic.allData ("zone_id = @zone_id", "", false, true, new KeyValuePair ("@zone_id", id));
			else
				opResult	= logic.allByPaging ( pageIndex, pageSize, "zone_id = @zone_id", "", false, true, new KeyValuePair ("@zone_id", id));

			if (opResult.status == BaseDAL.Base.EnumCommandStatus.success)
				_get_gatedevice_zone_id	= opResult.model as System.Data.DataTable;
		}
	#endregion
